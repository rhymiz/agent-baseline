...
Use `uvx agent-baseline@0.4.2 <command> <project>` or an installed matching `agent-baseline`. Requires uv and Python 3.11+; Git is optional for inventory and verification runs on macOS/Linux. The CLI has no model API dependency.
