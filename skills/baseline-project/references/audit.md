...
| Added value | Identify the decision changed by an instruction and what it adds beyond accessible contracts, examples, and default capabilities. Prefer links over duplicated explanations. A true statement, shorter file, or observed compliance alone does not establish improved work. Preserve explicit policy without requiring a performance experiment. |
...
| Policy scope | Distinguish portable authoring principles from the consumer owner's type, import, and public API policies. Preserve explicit restrictions within their scope; do not infer universal prohibitions from this package's conventions. |
...
| Instruction interaction | Trace conflicting rules, duplicate sources, and host routing. Preserve the canonical source and inspect precedence before proposing consolidation. Distinguish installation, metadata exposure, body loading, and behavior. Use available effective-context or trace evidence; a valid alias does not prove loading. |
...
For research-backed changes to these criteria, consult [research.md](research.md). It records source scope and limitations, not mandatory reading for every audit or authority over owner policy.
