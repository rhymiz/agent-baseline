...
For domain workflows, establish identity, lifecycle, ownership, valid states, boundary validation, failure semantics, and observable examples. Reuse concepts only when their invariants match. Keep internal types precise without widening, imports at module scope, and test-only branches out of production. Do not re-export names, including for backwards compatibility. Do not use casts or unexplained nullable state to hide contract errors.
