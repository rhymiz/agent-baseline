...
```sh
uvx agent-baseline@0.4.3 init . --agent codex --agent claude --agent grok
```

Choose the hosts you use; omit host flags for the explicit CLI workflow. Initialization installs the skill and creates an empty evidence-record draft. It preserves existing project instructions and does not invent domain rules, sources, or test commands.
...
The setup agent also configures [outcome follow-ups](skills/baseline-project/references/outcomes.md): it chooses an existing evidence store or a local Markdown fallback, writes the project's retrieval/update instructions, and creates an initial pending case when the inspected work supplies a real finding. Consumers do not need to design or write that workflow. Later comparable work supplies evidence of improvement, no change, regression, or an inconclusive result. The CLI's `init` command installs this procedure; completing the agent-led setup applies it to the project.

Invoke `$baseline-project` in Codex or `/baseline-project` in Claude Code or Grok Build, or select it in the host's skill picker. If the host does not discover local skills, ask it to run and follow:

```sh
uvx agent-baseline@0.4.3 skill show
```
...
```sh
uvx agent-baseline@0.4.3 skill show --file SKILL.md
uvx agent-baseline@0.4.3 skill show --file references/audit.md
```
...
Grok Build reads `.grok/skills` and also scans `.agents/skills`. Use `--agent grok` so `init` and `doctor` cover the native `.grok/skills` route. User-scope installs go to `~/.grok/skills`. The [0.2.0 native trials](docs/evaluation-0.2.0.md#grok-cli-follow-up) used an explicit skill read against Grok Build 1.0.13; discovery in other versions remains best effort.

## Daily use

```sh
uvx agent-baseline@0.4.3 doctor .
uvx agent-baseline@0.4.3 check .
uvx agent-baseline@0.4.3 verify .
```

`doctor` checks local Markdown links, referenced guidance, skill YAML, and selected host routes. Add `--agent codex --agent claude --agent grok` for native project discovery requirements. It understands Markdown references and code examples instead of searching prose with a link regex.
...
```sh
uvx agent-baseline@0.4.3 record .
uvx agent-baseline@0.4.3 verify .
```
...
```sh
uvx agent-baseline@0.4.3 skill link tooling/skills/team-engineering --agent codex --agent claude --agent grok
```
...
```sh
uvx agent-baseline@0.4.3 skill install --agent codex --scope user
```

Use `--agent claude` for Claude Code and `--agent grok` for Grok Build. Project discovery uses `.agents/skills` for Codex, `.claude/skills` for Claude Code, and `.grok/skills` for Grok Build. User discovery uses those directories under the user's home. The repository also includes a Codex plugin manifest; CLI installation does not register that plugin with a marketplace.

Installed files are persistent copies outside uv's cache. To upgrade a managed installation:

```sh
uvx agent-baseline@0.4.3 skill install --agent codex --scope user --upgrade
```
...
When evaluating added guidance, compare it with a minimal condition that retains required owner policy and essential commands. For authoring changes, freeze the generated baseline and test its use by a fresh consumer agent. Keep loading, task correctness, and engineering effort separate. The [research reference](skills/baseline-project/references/research.md) records primary sources and their limits; it is optional background, not a prerequisite for ordinary work or evidence of measured gains in this release.
